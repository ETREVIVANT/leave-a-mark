...
import { connectSocket, saveMarkViaSocket } from './socket.js';
...
// // Top-down orthographic, tighter frustum for “zoomed-in” feel
// const cam = new THREE.OrthographicCamera(
//   -(GRID_W*TILE)/6, (GRID_W*TILE)/6,
//   (GRID_H*TILE)/6, -(GRID_H*TILE)/6,
//   -100, 100
...
// // Optional play_area overlay (sits above ground)
// const playAreaTex = loader.load('./assets/play_area.PNG', (t)=>{
//   t.magFilter = t.minFilter = THREE.NearestFilter
//   t.colorSpace = THREE.SRGBColorSpace
// }, undefined, ()=>{})
...
// // Marks (2×2) above overlay, below player
// function addMarkSprite(dataURL, gx, gy){
//   const tex = new THREE.TextureLoader().load(dataURL)
//   tex.magFilter = tex.minFilter = THREE.NearestFilter
//   tex.colorSpace = THREE.SRGBColorSpace
...
// // D-pad (hold-to-repeat)
// const repeat = (fn)=>{ let id=0; return { start(){fn(); id=setInterval(fn,110)}, stop(){clearInterval(id)} } }
// function bindHold(sel, fn){
//   const el = document.querySelector(sel); if(!el) return
//   const rep = repeat(fn)
//   el.addEventListener('mousedown', rep.start)
...
// // Click/tap to open editor at current tile
// renderer.domElement.addEventListener('click', ()=> openEditorAt(target.x, target.y))
// // Star button
// btnPlace.onclick = () => openEditorAt(target.x, target.y)
